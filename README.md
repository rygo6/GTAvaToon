...
#### Cel-Shaded-Esque Toon Outlines
- The outline is done in a way similar to modern cel shaded games where it samples the normal and depth map. This is the first VRChat avatar shader that can draw a toon outline in this manner. This produces a much more detailed and accurate toon outline compared to the inverse hull method every other avatar shader in VRChat currently uses. It gives you accurate lines at mesh concavity, convexity and even arbitrary mesh intersections! See comparison in the image below for further explanation.

![BakeAO](Media~/outlinecomparison.png)

#### Levels Adjustment For How The World Lighting Effects You
- The lighting model is designed around a novel concept of `Local Lighting` and `World Lighting` specifically to give you greater control over your avatar's lighting and how the world lighting affects it. A major premise behind the design of this shader is that ***you cannot trust the world lighting!*** This intentionally breaks physically accurate lighting to give you controls that work like the Levels Adjustment in Photoshop to let you compress and constrain how the world lighting effects your avatar. This I believe is a superior technique as it lets you shift the minimum value upwards, compressing the values, so you can maintain a minimum brightness in dark worlds but not be overly bright in bright worlds. 
...
- I also tuned it so that lighting entirely from Light Probes looks roughly the same as lighting from Light Probes + Single Directional Light.
...
- Surfaces drawing an outline cannot also be transparent, however transparent surfaces can be on top of surfaces with outlines. It is possible to do alpha cutout on surfaces with outlines but I haven't implemented this yet.
...
- There are some subtle artifacts on depth outlines in some scenarios that I don't think can ever be fully solved. Most of the work on the first version of this shader actually went just to this, how to make this technique work in the limitations of VRChat. I've tried dozens of different methods and tweaks. The one I came up with I find is the best balance of detail, to artifacts, to ease of use, to performance.
...
- `Gradient Min Max` is the primary way you tune the thresholding of the Normal Outlines and Depth Outlines. Min should always be less than Max! But beware, this also affects the smoothness and anti-aliasing of the outline. If Min and Max are too close together, the line may appear too aliased and grainy, too far apart and may appear too blurry. As you adjust these it is good to zoom in on your avatar in unity GameView and look at how it is affecting the quality of the anti-aliasing on the line. _Note: The SceneView is not anti-aliased! So you must look in the GameView to get feedback on this._
